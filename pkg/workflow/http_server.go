...
func saveCompanyToFeishu(companyReq CompanyRequest) (string, error) {
    conf := config.GetConf()
    ...
    searchResp, err := client.Client.Bitable.V1.AppTableRecord.Search(
        context.Background(), req, larkcore.WithUserAccessToken(accessToken),
    )
    if err != nil {
        return "", fmt.Errorf("获取企业编号失败: %w\n", err)
    }
    
    if !searchResp.Success() {
        return "", fmt.Errorf("query serial failed: %s", larkcore.Prettify(searchResp.CodeError))
    }
    var response FeishuResponse
    if err = json.Unmarshal(searchResp.RawBody, &response); err != nil {
        return "", fmt.Errorf("解析 Serial 响应体失败: %w", err)
    }
    maxNumber := 0
    ...
    companySerial := maxNumber + 1
    fullName := fmt.Sprintf("%d-%s", companySerial, companyReq.CompanyName)
    ...
    if !insertResp.Success() {
        return "", fmt.Errorf("insert row failed: %s", larkcore.Prettify(insertResp.CodeError))
    }
    return fullName, nil
}
...
func createCompany(c *gin.Context) {
    ...
    fullName, err := saveCompanyToFeishu(companyReq)
    if err != nil {
        c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
        return
    }
